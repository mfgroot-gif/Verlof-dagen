...
with tab2:
    st.subheader("Maandelijkse Details")
    ...
    # Veilige, versimpelde weergave voor mobiel zonder HTML-grid
    st.markdown(f"### Status voor {gekozen_maand_naam} {jaar}")
    
    lijst_leeg = True
    for d in sorted(alle_maand_dagen.keys()):
        st.info(f"📅 **Dag {d}** — {alle_maand_dagen[d]}")
        lijst_leeg = False
    if lijst_leeg:
        st.success("Geen geplande verlofdagen of officiële feestdagen in deze maand.")

with tab3:
    st.subheader("Nieuwe mutatie invoeren")
    with st.form("vakantie_form", clear_on_submit=True):
        datum = st.date_input("Datum", datetime.date(jaar, 1, 1))
        categorie = st.selectbox("Type verzuim/verlof", ["Vakantie", "Nederlandse Feestdag", "Ziekte", "Bijzonder Verlof"])
        dagen = st.number_input("Aantal dagen", min_value=0.5, max_value=30.0, value=1.0, step=0.5)
        info = st.text_input("Omschrijving / Reden")
        
        submit = st.form_submit_button("Mutatie Opslaan")
        if submit:
